...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
    ...
    function getTotalVisits(label, value) {
        ...
        return new Promise((resolve, reject) => {
            console.log("label", label);
            console.log("value", value);
            admin.database().ref('/patient').update({ year: value, totalVisits: 0 });
            var count = 0;
            var intervalObject = setInterval(function () {
                fetchTotalVisits().then((totalVisit) => {
                    console.log(count, 'seconds passed');
                    console.log("TotalVisitResult", totalVisit);
                    count++;
                    if (totalVisit > 0) {
    ...
    function fetchTotalVisits() {
        return new Promise((resolve, reject) => {
            admin.database().ref('/patient').child('totalVisits').on('value', function (snapshot) {
